$(document).ready(function () {
    ...
    var not_allowed_date_range = ['register', 'login', 'verify.otp', 'password.request', 'user.plans', 'details.user', 'dashboard', 'user.ipv-screen', 'explore-operations.index', 'operations.index'];
    ...
    $('.evt_mark_all_as_read').on("click", function (e) {
        Swal.fire({
            title: ays_en_msg,
            text: ays_notification_en_msg,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#13153B',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'GET',
                    url: notifications_all_read_url,
                    dataType: 'json',
                    success: function (res) {
                        if (res.status == true) {
                            $('#ajax_header_notify_list').empty();
                            $('.headernoti_dot').removeClass('headernoti_dot');
                            toastr.success(res.message);
                        } else {
                            toastr.error(res.message);
                        }
...
{
    var dateRangVal = $('input[name="duration_date_range"]').val();
    if(dateRangVal) {
