...
$(document).on('click', '.active_inactve_tab', function (e) {
    e.preventDefault();
    var currency_name = $(this).attr('data-currency');
    $('.active_inactve_tab').removeClass('active');
    $(this).addClass('active');
    preferred_currency = USD;
    $('.add_currency_js').text(USD_SIGN);
    if (currency_name == USD) {
        $('body #gugs').removeClass('show active');
        $('body #amedlr').addClass('show active');
    } else {
        $('.add_currency_js').text(PYGH_SIGN);
        preferred_currency = PYGH;
        $('body #amedlr').removeClass('show active');
        $('body #gugs').addClass('show active');
    }
    calculationAllOperation();
});
            ...
            success: function (res) {
                ...
                if (res.status == true) {
                    ...
                    if (preferred_currency == USD) {
                        $('body #gugs-tab').removeClass('active');
                        $('body #gugs').removeClass('show active');

                        $('body #amedlr-tab').addClass('active');
                        $('body #amedlr').addClass('show active');

                        // $('body #group_offer_change_currency').prop('checked', false).trigger('change');
                    } else if (preferred_currency == PYGH) {
                        $('body #amedlr-tab').removeClass('active');
                        $('body #amedlr').removeClass('show active');

                        $('body #gugs-tab').addClass('active');
                        $('body #gugs').addClass('show active');
...
function singelOperationValidation(obj) {
    ...
    if (valid_form) {
        $('#btn_sent_offer_' + operation_id).removeClass('disabled');
    } else {
        $('#btn_sent_offer_' + operation_id).addClass('disabled');
    }
...
$(document).on('click', '.evt_submit_sent_offer', function (e) {
    ...
    const operation_id = self.attr('data-operation-id');
    const preferred_currency = self.attr('data-currency-type');
    var is_valid = singelOperationValidation(self);

    sentOffer(self, operation_id, seller_id, preferred_currency, (res_callback) => {
        console.log('res_callback');
    });
});

function sentOffer(btn_obj, operation_id, seller_id, preferred_currency, callback = () => { }) {
    var self = $(btn_obj);
    ...
    $.ajax({
        ...
        beforeSend: function() {
            ...
            $(self).html(btn_single_offer_loader);
        ...
        success: function (res) {
            if (res.status == true) {
                ...
                $('body #row_offer_remove_'+operation_id).css('transition', 'transform 0.5s, opacity 0.5s').addClass('hide-left');
                $('body #row_offer_remove_'+operation_id).fadeOut('slow', function () {
                    $(this).remove();
                });
                ...
                toastr.error(res.message);
                $(self).css('pointer-events', 'block');
                $(self).html(offer_cp_en_msg);
            }
        },
        error: function (xhr) {
            $(self).css('pointer-events', 'block');
        ...
        if (btn_name == 'btn_group_mipo' && is_group_offer_mipo_checkbox == true) {
            ...
            if (res && is_group_offer_mipo_checkbox == true) {
                if (offer_amount != '') {
                    ...
                    toastr.error(offer_amount_req_en_msg);
                }
            }
...
function removeOffer(obj, is_confirm = false) {
    ...
    var preferred_currency = self.attr('data-currency-type');
    if (is_confirm == true) {
        const res = confirm(ays_delete_send_offer_en_msg);
        if (res) {

            $(`body #row_offer_remove_${operation_id}:visible`).css('transition', 'transform 0.5s, opacity 0.5s').addClass('hide-right');
            $(`body #row_offer_remove_${operation_id}:visible`).fadeOut('slow', function () {
                $(this).remove();
            });
...
function calculationCurrentOperation(seller_id, operation_id) {
    ...
    operation_amount = getStrignToNumber(current_operation_obj.attr('data-operation-amount'));
    retention_amount = currency_inr($(`#retention_${operation_id}:visible`));

    real_time_offer_amount = currency_inr($(`#offer_amount_${operation_id}:visible`));

    var is_mipo_checked = $(`#mipo_verified_${operation_id}:visible`).is(":checked");

    var new_operation_amount = new_retention_amount = new_real_time_offer_amount = current_operation_retention_amount = 0;
    if (preferred_currency == USD) {
    ...
    current_operation_retention_amount = (operation_amount - retention_amount);
...
function overallOperationSummary() {
    ...
    var group_offer_retention_mipo = group_offer_retention = 0;
    $('.evt_group_offer_checkbox:visible').each(function (index) {
        ...
        group_offer_retention_mipo = currency_type_amount_sum(null, preferred_currency, $(`.grp_mipo_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
        ...
        group_offer_retention = currency_type_amount_sum(null, preferred_currency, $(`.grp_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
    ...
    $('.single_offer_amount:visible').each(function () {
        ...
        var single_offer_retention = single_offer_retention_mipo = 0;
        if ($(this).val() != '') {
            ...
            single_offer_retention_mipo = currency_inr($(`#retention_${operaion_id}[data-currency-type='${preferred_currency}']:visible`));
    
            var single_offer_amount_mipo_obj = $(`#offer_amount_${operaion_id}:visible`);
            var single_offer_mipo_real_time_amount = currency_inr($(`#offer_amount_${operaion_id}:visible`));
            var single_operation_mipo_amount = getStrignToNumber(single_offer_amount_mipo_obj.attr('data-operation-amount'));

            single_offer_retention = currency_inr($(`#retention_${operaion_id}[data-currency-type='${preferred_currency}']:visible`));
        
            var single_offer_amount_obj = $(`#offer_amount_${operaion_id}[data-currency-type='${preferred_currency}']:visible`);
            var single_offer_real_time_amount = currency_inr($(`#offer_amount_${operaion_id}[data-currency-type='${preferred_currency}']:visible`));
    ...
    $(main_object).each(function (index, seller_obj) {
        ...
        if (seller_obj.net_profit_amount != '') {
            overall_net_profit_total += parseInt(seller_obj.net_profit_amount);
