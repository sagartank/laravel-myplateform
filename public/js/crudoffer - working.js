...
$(document).on('change', '#group_offer_change_currency', function (e) {
    ...
    if ($(this).is(':checked') == true) {
        ...
        // $('#calculation_pyg').css('display', 'block');
        // $('#calculation_usd').css('display', 'none');
    } else {
        $('#pyg_tab').css('display', 'none');
        $('#usd_tab').css('display', 'block');

        // $('#calculation_pyg').css('display', 'none');
        // $('#calculation_usd').css('display', 'block');
    }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.status == true) {
                    ...
                    // console.log('modal_offer_seller_arr', modal_offer_seller_arr);
                    // $('body #calculation_div_row').hide();
                    /* if(res.record == false && res.currency == USD) {
                        $('body #group_offer_change_currency').prop('checked', true).trigger('change');
                    } */

                    /* if (preferred_currency == PYGH) {
                        $('body #group_offer_change_currency').prop('checked', true);
                    } */
...
function singelOperationValidation(obj) {
    ...
    offer_amount = getStrignToNumber($('#offer_amount_' + operation_id).val());
    retention_amount = getStrignToNumber($('#retention_' + operation_id).val());
    ...
    if (accept_below_document_amount_requested == 1) {
        if (document_amount > offer_amount) {
            obj_offer_amount.addClass('offer_error');
            // toastr.error(`Minimum document offer amount ${document_amount} `);
            valid_form = false;
        }
    }
...
function groupOperationValidation(obj) {
    ...
    retention_amount = getStrignToNumber($(`.grp_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
    offer_amount = getStrignToNumber($(`.grp_offer_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
...
function groupMipoOperationValidation(obj) {
    ...
    retention_amount = getStrignToNumber($(`.grp_mipo_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
    offer_amount = getStrignToNumber($(`.grp_mipo_offer_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
        ...
        $(`.group_mipo_operation_ids_${seller_id}[data-currency-type='${preferred_currency}']`).attr('data-currency-type', preferred_currency).each(function () {
            ...
            if (operaion_id != '') {
                form_data = {
                    ...
                    'operaion_retention': $(`#retention_${operaion_id}:visible`).val(),
                    ...
                    'operaion_offer_amount': $(`#offer_amount_${operaion_id}:visible`).val(),
...
function removeOffer(obj, is_confirm = false) {
    ...
    if (is_confirm == true) {
        ...
        if (res) {
            ...
            console.log('current_operation_amount', current_operation_amount);
    ...
    /* var operation_id = self.attr('data-operation-id');
     ...
     var total_offer_seller_mipo = $(".total_seller_mipo_group_" + seller_id).length;
     var total_offer_seller = $(".total_seller_group_" + seller_id).length;
 
     var total_offer_by_seller = (total_offer_seller_mipo + total_offer_seller);
 
     if (is_confirm == true) {
         const res = confirm('Are you sure, you want to permanent delete this?');
         if (res) {
             if ((total_offer_by_seller == 1)) {
                 $('#row_seller_name_remove_' + seller_id).remove();
                 $('#remove_seller_group_' + seller_id).remove();
                 $('#remove_mipo_group_by_seller_' + seller_id).remove();
             }
 
             if (total_offer_seller == 1 && offer_name == 'operation') {
                 $('#remove_seller_group_' + seller_id).remove();
             } else if (total_offer_seller_mipo == 1 && offer_name == 'mipo') {
                 $('#remove_seller_group_mipo_' + seller_id).remove();
             }
             $('#row_offer_remove_' + operation_id).remove();
         }
     } else {
         if ((total_offer_by_seller == 1)) {
             $('#row_seller_name_remove_' + seller_id).remove();
     ...
     const total_operations_offer = $(".evt_remove_offer").length;
     if (total_operations_offer == 0) {
         $('#group_modal').modal('hide');
         loadMoreExploreOperationsData();
     }
 
     var first_seller_obj = $(".evt_remove_offer").first();
 
     calculationCurrentOperation(first_seller_obj.attr('data-seller-id'), first_seller_obj.attr('data-operation-id'));
     $('.single_retention').first().focus();
...
function calculationAllOperation() {
    $('.evt_div_group_by_seller:visible').each(function (index) {
        ...
        $(`.retention_mipo_${seller_id}:visible`).each(function () {
            var self = $(this);
            if (self.val() != '') {
                mipo_retention_total += parseInt(self.val());
            }
        });
        ...
        $(`.offer_amount_mipo_${seller_id}:visible`).each(function () {
            var self = $(this);
            if (self.val() != '') {
                mipo_offer_amount_total += parseInt(self.val());
            }
        });
        $(`#seller_group_offer_mipo_amount_${seller_id}:visible`).val(mipo_offer_amount_total);
        ...
        $(`.retention_${seller_id}:visible`).each(function () {
            var self = $(this);
            if (self.val() != '') {
                operation_retention_total += parseInt(self.val());
            }
        });
        ...
        $(`.offer_amount_${seller_id}:visible`).each(function () {
            var self = $(this);
            if (self.val() != '') {
                operation_amount_total += parseInt(self.val());
            }
        });
        $(`#seller_group_offer_amount_${seller_id}:visible`).val(operation_amount_total);
...
function overallOperationSummary() {
    ...
    $('.evt_group_offer_checkbox:visible').each(function (index) {
        ...
        var group_offer_retention_mipo = getStrignToNumber($(`.grp_mipo_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
        var group_offer_amount_mipo_obj = $(`.grp_mipo_offer_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`);
        var group_offer_mipo_real_time_amount = getStrignToNumber(group_offer_amount_mipo_obj.val());
        var group_operation_mipo_amount = getStrignToNumber(group_offer_amount_mipo_obj.attr('data-operation-total-amount'));

        var group_offer_retention = getStrignToNumber($(`.grp_ret_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`).val());
        var group_offer_amount_obj = $(`.grp_offer_amount_${seller_id}[data-currency-type='${preferred_currency}']:visible`);
        var group_offer_real_time_amount = getStrignToNumber(group_offer_amount_obj.val());
        ...
        if ($(this).is(':visible') && $(this).prop('checked') == true) {
            if (offer_type == OFFER_TYPE_MIPO) {
                ...
                console.log(total_retention_amount);
    ...
    $('.single_offer_amount:visible').each(function () {
        ...
        if ($(this).val() != '') {
            ...
            var single_offer_retention_mipo = getStrignToNumber($(`#retention_${operaion_id}[data-currency-type='${preferred_currency}']:visible`).val());
            var single_offer_amount_mipo_obj = $(`#offer_amount_${operaion_id}:visible`);
            var single_offer_mipo_real_time_amount = getStrignToNumber(single_offer_amount_mipo_obj.val());
            var single_operation_mipo_amount = getStrignToNumber(single_offer_amount_mipo_obj.attr('data-operation-amount'));

            var single_offer_retention = getStrignToNumber($(`#retention_${operaion_id}[data-currency-type='${preferred_currency}']:visible`).val());
            var single_offer_amount_obj = $(`#offer_amount_${operaion_id}[data-currency-type='${preferred_currency}']:visible`);
            var single_offer_real_time_amount = getStrignToNumber(single_offer_amount_obj.val());
