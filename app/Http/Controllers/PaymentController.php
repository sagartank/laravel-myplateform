...
class PaymentController extends Controller
{
    public function paymentReturn(Request $request,$shop_process_id){
        ...
        //if($planSlug && $confirmation['response'] == "S"){
            ...
            $user->plan_status = 1;
            $user->account_type = config('constants.ACCOUNT_TYPE_ENTERPRISE');
            $user->plan_ends_at = $plan->expired_date;
            $user->save();

            //assign role according to request
            if($role){
                $user->roles()->syncWithPivotValues([$role->id], ['user_type' => 'App\Models\User']);
            }
        ...
        if($dealMipoCommissionDealsSlug) 
        {
            ...
            if($offer_id > 0 && $offer->is_mipo_commission_payment == 'Yes' && isset($step_id) && $step_id > 0) 
            {
                $result_deals_tracking = app('deals_tracking')->getOperationByTracking($offer_id);
                $all_tracking_steps = $result_deals_tracking['all_tracking_steps'] ?? [];
