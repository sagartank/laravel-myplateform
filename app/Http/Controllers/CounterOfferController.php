...
use App\Mail\CounterOfferReceivedbyInvestor;
use App\Mail\OfferApprovedAskingtoSignContractBuyer;
use App\Mail\OfferApprovedAskingtoSignContractSeller;
use App\Mail\PayComissiontoMipo;
use App\Mail\PaytoSeller;
use App\Mail\DocumentSendingBuyerAndSeller;
use App\Notifications\DealCloseApproved as NotificationAdminDealCloseApproved;
...
class CounterOfferController extends Controller
{
    ...
    {
        $is_send_notification = config('constants.SEND_NOTIFICATION');
        ...
        if($request->ajax())
        {
            ...
            if( (int) $counter_offer_amount > $offer_with_operation_amount || is_null($offer_query)) {
                $response = [
                    'status' => false,
                    'message' => __('Counter offer amount must less than operation amount'),
                    'data' => []
                ];
            ...
            try {
                    ...
                    if($is_send_notification == true) {
                        ...
                        if($user_login_id != $update->buyer_id) {
                            $seller_user_obj = app('common')->getUserEmail($user_login_id);
                            Notification::send($seller_user_obj, new NotificationsReceivingCounterOfferBorrower(app()->getLocale()));
                        }
                    }
                
                    Mail::to($user_obj->email)->send(new CounterOfferReceivedbyInvestor($user_obj->name));

                    app('common')->addLogs('Counter Offer Received by Investor Email', $user_obj->id);
                    ...
                    $response = [
                        'status' => true,
                        'message' => __('Counter offer sent successfully'),
                        'data' => []
                    ];
                } catch (\Throwable $th) {
                        DB::rollBack();
        ...
        if($request->ajax())
        {
            ...
            if($update)
            {
                ...
                if($offer_status == 'Revert') {
                    $msg = __('Offer revert back successfully');
                    ...
                    if($offer_status == 'Approved') {
                        $msg = __('Offer approved successfully');
                        ...
                        if($offer_status == 'Rejected') {
                            $msg = __('Offer rejected successfully');
                        }
                        $update->offer_status = $offer_status;
                        $update->save();
                    }
                ...
                $offer_result = Offer::where('id', $offer_id)->with('operations:id,seller_id,operation_number')->where('offer_status', 'Approved')->select('id', 'slug', 'offer_status', 'buyer_id')->first();
                ...
                if($offer_result) {
                        ...
                        Mail::to($buyer_obj->email)->send(new OfferApprovedAskingtoSignContractBuyer($buyer_obj->name, $buyer_link));

                        app('common')->addLogs('offer approved asking to sign contract buyer Email', $buyer_obj->id);


                        $seller_obj = app('common')->getUserEmail($offer_result->operations->first()->seller_id);
                        
                        $seller_link = route('deals.details', [$offer_result->slug, 'seller']);

                        Mail::to($seller_obj->email)->send(new OfferApprovedAskingtoSignContractSeller($seller_obj->name, $seller_link));

                        app('common')->addLogs('offer approved asking to sign contract buyer Email', $seller_obj->id);
                        ...
                        if($is_send_admin_notification == true) {
                            ...
                            Notification::send($admin_obj, new NotificationAdminDealCloseApproved($user_obj));
                        }
                }
        ...
        try {
            ...
            if($result->preferred_payment_method !='Cash') {
                ...
                if($is_user_bank == 0) {
                    ...
                    $response = [
                        'status' => false,
                        'message' => __('Please add bank details'),
                        'data' => []
                    ];
                    return response()->json($response);
                }
                ...
                if(($user_type == 'seller' || $user_type == 'buyer') && $user_contract_sing->count() > 0) {
                    ...
                    foreach ($user_contract_sing as $key => $user_contract_sing_val) {
                        ...
                        if(is_null($save)) {
                            ...
                            Mail::to($user_obj->email)->send(new MailSignContractOtp($otp));
            ...
            if($user_login_id == $result->buyer_id && $user_type = 'buyer') {
                ...
                Mail::to($buyer_obj->email)->send(new MailSignContractOtp($otp));
                ...
                if($user_type == 'seller' && $user_login_id!='') {
                    ...
                    Mail::to($seller_obj->email)->send(new MailSignContractOtp($otp));
            ...
            if($otp_resend == 'false') {
                ...
                if($result->operations->first()->operation_type == 'Invoice') {
                    $dhtml = view('deals.ajax.contract.ajax-confrim-offer-contract-invoice', $data)->render();
                } else {
                    $dhtml = view('deals.ajax.contract.ajax-confrim-offer-contract-cheque-contract-other', $data)->render();
                }
        ...
        if($request->ajax())
        {
            ...
            try {
                ...
                if($update && $user_login_id)
                {
                    ...
                    if($user_login_id == $update->buyer_id) {
                        $user_type = 'buyer';
                        $user_obj = app('common')->getUserEmail($user_login_id);
                        $buyer_link = route('deals.details', [$update->slug, 'buyer']);
                        Mail::to($user_obj->email)->send(new PayComissiontoMipo($user_obj->name, $buyer_link));
                    } else {
                        $user_obj = app('common')->getUserEmail($user_login_id);
                        $seller_link = route('deals.details', [$update->slug, 'seller']);
                        Mail::to($user_obj->email)->send(new PaytoSeller($user_obj->name, $seller_link));
                    }
                    ...
                    $msg = __('Contract signed successfully');
                ...
                if(!empty($save_deals_contract->buyer_id) && !empty($save_deals_contract->seller_id))
                {
                    ...
                    $data['deals_contract_buyer_and_seller'] = $deals_contract;
                    $data['user_contract_sing_otps'] = userContractSingOtp::
                        with('user:id,issuer_id,name,email', 'user.issuer:id,ruc_text_id,ruc_code_optional,company_name,commercial_name')
                        ->where('offer_id', $offer_id)
                        ->get();
                    
                    $fileName = $result->id.time().".pdf";
                    
                    /* if($result->operations->first()->operation_type == 'Invoice') {
                        $pdf = Pdf::loadView('operations.contract-pdf.confrim-offer-contract-invoice', $data);
                    } else {
                        $pdf = Pdf::loadView('operations.contract-pdf.confrim-offer-contract-cheque-contract-other', $data);
                    } */

                    if($result->operations->first()->operation_type == 'Invoice') {
                        $pdf = Pdf::loadView('deals.contract-pdf.confrim-offer-contract-invoice', $data);
                    } else {
                        $pdf = Pdf::loadView('deals.contract-pdf.confrim-offer-contract-cheque-contract-other', $data);
                    }
                    ...
                    $user_buyer_obj = app('common')->getUserEmail($save_deals_contract->buyer_id);
                    Mail::to($user_buyer_obj->email)->send(new DocumentSendingBuyerAndSeller($user_buyer_obj->name));
                    Notification::send($user_buyer_obj, new NotificationsSendContractFileSellerAndBuyer($attach));
                    
                    /* send Seller */
                    $user_seller_obj = app('common')->getUserEmail($save_deals_contract->seller_id);
                    // Mail::to($user_seller_obj->email)->send(new DocumentSendingBuyerAndSeller($user_seller_obj->name));
                    Notification::send($user_seller_obj, new NotificationsSendContractFileSellerAndBuyer($attach));
                }
