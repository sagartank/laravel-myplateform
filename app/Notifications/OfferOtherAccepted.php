...
class OfferOtherAccepted extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        $subject = 'OTHER OFFERS ACCEPTED';

        $data['user_level_name'] = strtoupper($this->user_level_name);

        $content = view('emails.offer-other-accepted', $data)->render();
        // logger( $notifiable);    
