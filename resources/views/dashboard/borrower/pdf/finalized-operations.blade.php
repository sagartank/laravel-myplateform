...
<body>
	<div class="pdf-wrapper">
		...
		<div class="pdf-body">
			<div class="h2">{{ __('Finalized Operations')}}</div>
							...
							<tr>
								<td class="pdf_income_lista_col">
									<table class="pdf_income_lista_box">
										<tr>
											<td class="h4">{{ __('Total documents sold') }}</td>
										</tr>
										<tr>
											<td class="income_price pdf_color_green">{{ app('common')->currencyBySymbol($req_param['currency_type']).' '.app('common')->currencyNumberFormat($req_param['currency_type'], $total_finalized_doc_sold) }}</td>
										</tr>
									</table>
								</td>
								<td class="pdf_income_lista_col">
									<table class="pdf_income_lista_box">
										<tr>
											<td class="h4">{{ __('Accepted Offers') }}</td>
										</tr>
										<tr>
											<td class="income_price">{{ app('common')->currencyBySymbol($req_param['currency_type']).' '.app('common')->currencyNumberFormat($req_param['currency_type'], $total_finalized_doc_accepted) }}</td>
										</tr>
									</table>
								</td>
                                <td class="pdf_income_lista_col">
									<table class="pdf_income_lista_box">
										<tr>
											<td class="h4">{{ __('Average Discount') }}</td>
										</tr>
										<tr>
											<td class="income_price pdf_color_green">{{ app('common')->currencyBySymbol($req_param['currency_type']).' '.app('common')->currencyNumberFormat($req_param['currency_type'], $total_finalized_avg_discount) }} %</td>
										</tr>
									</table>
								</td>
							</tr>
																		...
																		<table>
																			<tr>
																				<td style="padding: 0 10px 0 0 !important;">
																					<div class="pdf_chaque_name">{{ $finalized_doc_sold->operation_type_number.' - '.$finalized_doc_sold->seller->name  }}</div>
																					<div class="pdf_chaque_ownar">{{ $finalized_doc_sold->issuer->company_name }}</div>
																				</td>
																...
																<tr>
																	<td style="padding: 5px 10px;">
																		<table>
																			<tr>
																				<td style="padding: 0 10px 0 0 !important;">
																					<div class="pdf_chaque_name">{{ $finalized_doc_accepted->operation_type_number.'-'.$finalized_doc_accepted->seller->name  }}</div>
																					<div class="pdf_chaque_ownar">{{ $finalized_doc_accepted->issuer->company_name }}</div>
																				</td>
																				<td class="pdf_chaque_amount" style="padding: 0 0 0 10px !important; vertical-align: top;">
									...
									<table>
										<tr>
											<td class="right_row_pdf">
												<table>
													<tr>
                                                        @if($line_chart_image &&  $line_chart_image!='')
														<td>
                                                            <img style="width: auto; height: auto; max-width: 100%; " src="{{ $line_chart_image }}" alt="no-image">
														</td>
                                                        @endif
													</tr>
												</table>
