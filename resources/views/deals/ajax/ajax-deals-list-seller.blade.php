    ...
    <div class="deals_section_wrapper">
        @foreach ($deals_record as $key => $deals)
            ...
            <div class="deals_sold_caption">
                <div class="left_part">
                    <div class="cap_box">
                        <a href="{{ route('deals.details', [$deals->slug, 'seller']) }}" class="name text-14-medium">{!! __($deals->operations->first()?->operation_type_number) !!}</a>
                    ...
                    <a href="{{ route('deals.details', [$deals->slug, 'seller']) }}" class="company_name text-14-medium">{!! __($deals->operations->first()?->issuer->company_name) !!}</a>
                    <div class="imagesbox">
                        <ul class="first">
                ...
                <div class="right_part">
                    ...
                    <div class="second_right">
                        <p class="text-14-medium">
                            @if(app()->getLocale() == 'es')
                            {{ ($deals->offers_logs->count() > 0) ? $seller_steps[$deals->offers_logs->first()?->title] : '-' }}
                            @else
                ...
                <a href="{{ route('deals.details', [$deals->slug, 'seller']) }}" class="full_link"></a>
            </div>
            {{-- mobile screen --}}
                ...
                <div class="left_part">
                    <div class="cap_box">
                        ...
                        <div class="second_sold">
                            <a href="javascript:;" class="payer_name text-12-medium">{!! __($deals->operations->first()?->seller->name) !!}</a>
                            <i><img src="{{ asset('images/mipo/deals-img2.svg') }}" alt="no-image"></i>
                            <a href="javascript:;" class="seller_name text-12-medium">{!! __($deals->buyer?->name) !!}</a>
                            <span class="seller_btn text-12-medium">{!! __('Buyer') !!}</span>
                        </div>
                        <a href="{{ route('deals.details', [$deals->slug, 'seller']) }}" class="company_name text-14-medium">{!! __($deals->operations->first()?->issuer->company_name) !!}</a>
                    </div>
    ...
    @if ($last_page > 1)
        <div class="bottom_pageSec">
            <div class="expdoc_pager paginate_seller">
                {!! $deals_record->links() !!}
            </div>
            @if ($last_page > 1)
                <div class="exp_sortwrp paginate_seller">
                    <span>{!! __('Go to Page') !!}</span>
                    <input type="number" name="offer_page_no" id="got_to_page_seller">
                    <a href="javascript:void(0)" class="evt_btn_go_page_no" data-active-name="seller" data-last-page="{{ $last_page }}">{!! __('Go') !!} <i><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
