<x-guest-layout>
    @section('pageTitle', 'User IPV Screen') 
...
<style>
	...
	<section>
        ...
        <form action="{{ route('store.ipv-photo') }}" name="user-ipv-screen-form" method="POST" enctype="multipart/form-data">
            @csrf   
            <input type="hidden" name="ipv_image" id="ipv_image">
            <div class="ipv_main_page ipv_click">
                <div class="cam_not_section" id="cam_not_section">
                    <div class="cam_not">
                        <div class="camera">
                            <i><img src="{{asset('images/mipo/ipv-not-found.svg')}}" alt="mipo"></i>
                            <p class="text-14-medium">{!! __('The camera is not active') !!}</p>
                            <div class="btn_section">
                                <a href="javascript:;" class="text-14-medium" id="btn-allow-camera">{!! __('Allow Camera') !!}</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="camera_screen" id="camera_screen_inner">
                    <div class="camera_screen_inner">
                        <video id="ipv_video">{{ __('Video stream not available.') }}</video>
                        <img src="javascript:;" style="display: none" id="ipv_image_preview">
                        <canvas id="canvas" style="display: none"></canvas>
                    </div>
                </div>
                <div class="camera_controll" id="startbuttonDiv">
                    <div class="camera_icon">
                        <a href="javascript:;" id="startbutton">
                    ...
                    <div class="camera_txt text-16-medium">
                        {!! __('Click on the button to capture photo') !!}
                    </div>
                </div>
                <div class="camera_controll cam_photo" style="display: none;" id="retakebuttonDiv">
                    <div class="container">
                        <div class="camera_controll_inner">
                            <div class="camera_icon_reload">
                                <a href="javascript:;" id="retakebutton">
 ...
 */
            ...
            (() => {
                ...
                function startup() {
                    ...
                    navigator.mediaDevices.getUserMedia({ video: true, audio: false }).then((stream) => {
                        $('#cam_not_section').hide();
                        $('#camera_screen_inner').show();
                        $('#startbuttonDiv').show();
                            localstream = stream;
                        ...
                        .catch((err) => {
                            $('#cam_not_section').show();
                            $('#camera_screen_inner').hide();
                            $('#startbuttonDiv').hide();
        ...
        <script>
            $(document).on('click', '#btn-allow-camera', function (e) {
                alert('please open front camera');
            });
        </script>
