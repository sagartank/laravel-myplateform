   @php
     $img_slider = [0 => 'first', 1 => 'second' , 2 => 'third'];
   @endphp
   @if (isset($offers) && $offers->count() > 0)
        @foreach ($offers as $key => $offer)
            @php
                if ($offer->offer_status == 'Counter') {
                    $offer_expire_hour = $offer->counter_offers()->first()->counter_offer_expire_hour;
                    $payment_type = $offer->counter_offers()->first()->preferred_payment_method;
                    $amount = $offer->counter_offers()->first()->amount;
                } else {
                    $amount = $offer->amount;
                    $payment_type = $offer->preferred_payment_method;
                    $offer_expire_hour = $offer->offer_expire_hour;
                }
                $group_operation_amount = 0;
            @endphp
            
            @if ($offer->offer_type == 'Group')
                @php
                    $high_offer_result = $offer->operations->first()->offers->first();
                    $accept_below_requested = $offer->operations->first()->accept_below_requested ?? '0';
                @endphp
            <div class="offered_opboxWrap total_offered_operations" data-screen="laptop">
                <div class="offered_opbox">
                    <div class="leftpart">
                        <div class="multiimgbox">
                            <div class="imgbox">
                            @if ($offer->operations->first()->documents)
                                    @foreach ($offer->operations->first()->documents as $key => $document)
                                        @if ($document->document_url != '')
                                            @if ($document->extension != 'pdf')
                                                    <div class="{{ $img_slider[$key] }}">
                                                        <img src="{{ $document->document_url }}" alt="no-image">
                                                    </div>
                                            @endif
                                        @endif
                                    @endforeach
                            ...
                            <div class="nameDate">
                                <strong class="text-12-medium">• {!! __('Last Offer') !!}</strong>
                                <span class="text-12-medium">{!! $offer->offer_created_date_iso !!}</span>
                            </div>
                        </div>
                        <div class="lastofr">
                        <div class="opecheck_all_wrp text-16-medium">
                            <a href="javascript:;" class="codeid">{!! __('GROUP OFFER') !!}  -  {{ $offer->operations->first()->operation_type_number }}</a>
                            <span class="cash text-16-medium">{!! __($offer->operations->first()->preferred_payment_method) !!}</span>
                            <a href="{{ route('profile.public-seller', $offer->operations->first()->seller->slug) }}" class="name">{{ $offer->operations->first()->seller->name }}</a>
                            <div class="imgbox"><i><img src="{{ asset('images/mipo/singlestr.png') }}" alt="no-image"></i>
                                <span class="text-16-medium">{{ floor($offer->operations->first()->seller?->ratings_avg_rating_number)}}{!! __('/5 ('. $offer->operations->first()->seller?->ratings_count .')') !!}</span>
                            </div>
                        </div>
                        <div class="viewAll_op">
                            <a href="javascript:;" data-offer-id="{{ $offer->id }}" class="viewalltag evt_group_offer_by_operation">
                                <span class="text-12-medium">{!! __('View Operations') !!}</span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16" fill="none">
                                    <g clip-path="url(#clip0_3317_3420)">
                                        <path d="M7.41406 10.3346L10.0807 7.66797" stroke="#0D6EFD" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M7.41406 5L10.0807 7.66667" stroke="#0D6EFD" stroke-linecap="round" stroke-linejoin="round"/>
                                    </g>
                                    <defs>
                                ...
                                <div class="bestOfr">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14" viewBox="0 0 15 14" fill="none">
                                        <g clip-path="url(#clip0_1994_64900)">
                                            <path d="M5.34008 7.00056H3.24825C3.13289 7.00054 3.02014 6.96631 2.92423 6.90221C2.82832 6.83811 2.75358 6.74702 2.70944 6.64044C2.6653 6.53387 2.65374 6.4166 2.67624 6.30346C2.69874 6.19032 2.75428 6.08639 2.83583 6.00481L6.67766 2.16298C6.78706 2.05362 6.9354 1.99219 7.09008 1.99219C7.24476 1.99219 7.39311 2.05362 7.5025 2.16298L11.3443 6.00481C11.4259 6.08639 11.4814 6.19032 11.5039 6.30346C11.5264 6.4166 11.5149 6.53387 11.4707 6.64044C11.4266 6.74702 11.3518 6.83811 11.2559 6.90221C11.16 6.96631 11.0473 7.00054 10.9319 7.00056H8.84008V8.75056H5.34008V7.00056Z" stroke="#0D6EFD" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M5.34009 12.25H8.84009" stroke="#0D6EFD" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M5.34009 10.5H8.84009" stroke="#0D6EFD" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1994_64900">
                                            <rect width="14" height="14" fill="white" transform="translate(0.0900879)"/>
                                            </clipPath>
                                        </defs>
                                        </svg>
                    ...
                    <div class="offerd-status-box-group">
                ...
                <div class="offeredop_nav">
                    <div class="lftbox">
                        <div class="links">
                            @if ($offer->offer_status == 'Pending' || $offer->offer_status == 'Rejected')
                                <a href="javascript:;" class="redtxt text-12-medium evt_change_status" data-offer-id="{{ $offer->id }}" data-status="Revert">{!! __('Revert Back') !!}</a>
                            @endif
                            @if ($offer->offer_status == 'Pending' || $offer->offer_status == 'Rejected' || $offer->offer_status == 'Counter')
                            <a href="javascript:;" class="bluetxt text-12-medium evt_update_offer_popup" data-offer-id="{{ $offer->id }}">{!! __('Update Offer') !!}</a>
                            @endif

                            <a href="javascript:;" class="greytxt text-12-medium evt_offered_list" data-offer-id="{{$offer->id}}" data-currency-name="{{ $offer->operations()->first()->preferred_currency }}">{!! __('View History') !!}</a>
                        </div>
                    </div>
                    <div class="rhgtbox">
                    @if ($offer->offer_status == 'Counter')
                        <div class="tag">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                                ...
                                <path d="M9.33325 2.33594H11.6666V4.66927" stroke="#707070" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M8.75 5.2526L11.6667 2.33594" stroke="#707070" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M2.33325 11.6667L5.24992 8.75" stroke="#707070" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M9.33325 11.6693H11.6666V9.33594" stroke="#707070" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M2.33325 2.33594L11.6666 11.6693" stroke="#707070" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
                                </g>
                                <defs>
             ...
             {{-- mobile section --}}
            @include('offered-operations.ajax.mobile.group-offered-operations-list',
            [
                'offer' => $offer,
                'currency_symblos' => $currency_symblos,
                'high_offer_result' => $high_offer_result,
                'img_slider' => $img_slider,
                'amount' => $amount,
                'payment_type' => $payment_type,
                'offer_expire_hour' => $offer_expire_hour,
                'group_operation_amount' => $group_operation_amount,
            ])
            @endif
            @if ($offer->offer_type == 'Single')
                ...
                <div class="offered_opboxWrap total_offered_operations" data-screen="laptop">
                    <div class="offered_opbox">
                        <div class="leftpart">
                            ...
                            <div class="lastofr">
                            <div class="opecheck_all_wrp text-16-medium">
                                <a href="{{ route('explore-operations.details', $offer->operations->first()->slug) }}" class="codeid">
                                    {{ $offer->operations->first()->operation_type_number }}
                                </a>
                                <span class="cash text-16-medium">{{ __($offer->operations->first()->preferred_payment_method) }}</span>
                                <a href="{{ route('profile.public-seller', $offer->operations->first()->seller->slug) }}" class="name"> {{ app('common')->lockOperationDetail($offer->operations->first(), []) }}</a>
                                <div class="imgbox"><i><img src="{{ asset('images/mipo/singlestr.png') }}" alt="no-image"></i>
                                    <span class="text-16-medium">{{ floor($offer->operations->first()->seller?->ratings_avg_rating_number) }}{!! __('/5 ('. $offer->operations->first()->seller?->ratings_count .')') !!}
                                    </span>
                                </div>
                            </div>
                            <div class="company text-16-medium">
                                <a href="{{ route('profile.public-payer-profile', $offer->operations()->first()->issuer?->slug) }}"> {{ $offer->operations()->first()->issuer->company_name ?? '-' }}</a>
                            </div>

                            <div class="resource_wrap">
                                <ul class="first text-14-medium">
                                    <li class="ficon"><i><svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="rightpart">
                            <div class="userLocation">
                                ...
                                <ul>
                                    @if ($offer->operations()->first()->seller->address_verify == 'Yes')
                                        <li><i><img src="{{ $offer->operations()->first()->seller->address_verify_img }}" title="{{ __('Address verified')}}" alt="{{ __('Address verified')}}"></i></li>
                                    @endif
                                    <li><i><img src="{{ app('common')->userLevelImage($offer->operations()->first()->seller->user_level) }}" title="user level" alt="user level"></i></li>
                                </ul>
                                
                                <p class="text-16-medium">{!! __($offer->operations()->first()->seller->city?->name ?? 'Unknown City') !!}</p>

                            </div>
                            <div class="expireDay text-14-medium" data-info="operation expire_at">
                            ...
                            <div class="down_share">
                                <ul>
                                    <li><a href="javascript:;" class="evt_download_pdf_btn" data-href="{{ route('export.explore-operation-detail', $offer->operations->first()->slug) }}"
                ...
                {{-- mobile section --}}
                @include('offered-operations.ajax.mobile.single-offered-operations-list',
                [
                    'offer' => $offer,
                    'currency_symblos' => $currency_symblos,
                    'high_offer_result' => $high_offer_result,
        ...
        {{-- pagination:st --}}
        @if ($last_page > 1)
        <div class="offered-op-pagination">
            <div class="bottom_pageSec">
                ...
                @if ($last_page > 1)
                    <div class="exp_sortwrp">
                        ...
                        <input type="number" name="page_no" id="page_no">
    ...
    @else
        ...
        <div class="ope_notfoundWrap">
            <div class="imgbox">
                <div class="day"><img src="{{ asset('images/mipo/oferdopday.png') }}" alt="no-image"></div>
                <div class="night"><img src="{{ asset('images/mipo/oferdopdaynight.png') }}" alt="no-image"></div>
            <strong class="text-20-semibold">{!! __('No offers have been sent') !!}</strong>
            <p class="text-16-medium">{!! __('Search for operations to bid') !!}</p>

            <div class="newoprationBtn">
                <a href="{{ route('explore-operations.index')}}" class="text-16-medium"><i><img src="{{ asset('images/mipo/expsearch.svg') }}" alt="no-image"></i> {!! __('Explore Operation') !!}</a>
            </div>
        </div>
