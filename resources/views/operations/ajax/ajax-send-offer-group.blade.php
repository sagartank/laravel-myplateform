@if(isset($offers) && $offers->count() > 0)
    <div class="my_fourthopera">
        ...
        <div href="javascript:;" class="opera_oction">
            <div class="opera_leftpart">
                <div class="oction_head">
                    <h6 class="text-14-medium"> {{ app('common')->lockOfferDetail($offers_details, ['buyer_name']) }}</h6>
                </div>
                <p class="text-14-medium">
                    {{ __(' Expire in')}} {{ $offers_details->offer_expire_hour}} {{ __('hour')}}
                </p>
            </div>
            <div class="opera_rightpart">
                <h6 class="text-14-medium">
                    {{ app('common')->currencyBySymbol($offers->first()->preferred_currency) .''. app('common')->currencyNumberFormat($offers->first()->preferred_currency, $offers_details->amount )}}
                </h6>
                <p class="text-14-medium">{{ __($offers_details->preferred_payment_method) }}</p>
                <div class="btnbox evt_hide_show_approved_reject_div">
                    {{-- @permission('my-operations-can-reject') --}}
                    <a href="javascript:;" onclick="offerAcceptReject(this, 'Rejected')" data-offer-id="{{ $offers->first()->id }}" class="rejectbtn text-14-medium add_offer_id">{!! __('Reject') !!}</a>
                    {{-- @endpermission --}}
                    {{-- @permission('my-operations-can-accept') --}}
                    <a href="javascript:;" onclick="offerAcceptReject(this, 'Approved')" data-offer-id="{{ $offers->first()->id }}" class="approvebtn text-14-medium add_offer_id">{!! __('Approve') !!}</a>
                    {{-- @endpermission --}}
                </div>
            </div>
        </div>
        <form action="{{route('counter-offer.ajax-save-counter-offer')}}" novalidate="novalidate" method="post" name="counter_offer_form" id="counter_offer_form">
            <input type="hidden" name="offer_id" id="select_offer_id" value="{{ $offers->first()->id }}">
            <input type="hidden" name="offer_amount_hidden" id="offer_amount_hidden" value="{{ $offers_details->amount }}">
                ...
                <div class="count-body">
                    <div class="offerbox">
                        ...
                        <div class="offer_field">
                            ...
                            <div class="inputfield">
                                <div class="ofr">
                                    <input type="text" name="counter_offer_amount" required id="counter_offer_amount" minlength="1" data-msg-required="{{ __('The amount is required.') }}" value="" placeholder="Amount" class="counter_offer_amount">
        ...
        <div class="previously">
            <div class="imgbox text-12-medium">
                {!! __('Previously') !!}   {{ __('Expire in')}} {{ $offers_details->offer_expire_hour}} {{ __('hour')}} -    {{ app('common')->currencyBySymbol($offers->first()->preferred_currency) .''. app('common')->currencyNumberFormat($offers->first()->preferred_currency, $offers_details->amount )}}
                <i><img src="{{ asset('images/mipo/offtime.svg') }}" alt="no-image"></i>
            </div>
        </div>
        <div class="gr_offersbtn">
            <a href="javascript:;" data-offer-id="{{ $offers_details->id }}" class="text-14-medium evt_group_offer_by_operation">{!! __('Group Offer') !!}<i><img src="{{ asset('images/mipo/arrowrightblue.svg') }}" alt="no-image"></i></a>
        </div>
    </div>
@endif
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-12-medium">
                                    {{ app('common')->currencyBySymbol($offers->first()->preferred_currency)}} {{ app('common')->currencyNumberFormat($offers->first()->preferred_currency, $offers_history->amount) }}
                                </td>
                                <td class="text-12-medium">
                                    {{ app('common')->currencyBySymbol($offers->first()->preferred_currency)}} {{ app('common')->currencyNumberFormat($offers->first()->preferred_currency, $offers_history->retention) }} </td>
                                </td>
