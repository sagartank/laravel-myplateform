...
<head>
	...
	<style>
        ...
        * {
		...
		.pdf-header {padding: 0 20px 40px;}
			...
			<div class="pdf-header">
				<table>
					<tr>
						...
						<td>
							<div class="head-date">
								<div class="date-title">Operation Report</div>
					...
					<td width="80%">
						<table>
							...
							<tr>
								...
								<td><strong>{{ $operation->responsibility }} Resource</strong></td>
							</tr>
							<tr>
							...
							<tr>
								<td>Seller</td>
								<td>
									<strong>{{ $operation->seller->name }}</strong>
									<span class="rating">
										<ul>
											<li class="active">
												<svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.45793 8.87791L2.37193 10.5004L2.96143 7.06391L0.461426 4.63041L3.91143 4.13041L5.45443 1.00391L6.99743 4.13041L10.4474 4.63041L7.94743 7.06391L8.53693 10.5004L5.45793 8.87791Z" stroke="#FFC107" stroke-width="0.75" stroke-linecap="round" stroke-linejoin="round"/></svg>
											</li>
											<li class="active">
												<svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.45793 8.87791L2.37193 10.5004L2.96143 7.06391L0.461426 4.63041L3.91143 4.13041L5.45443 1.00391L6.99743 4.13041L10.4474 4.63041L7.94743 7.06391L8.53693 10.5004L5.45793 8.87791Z" stroke="#FFC107" stroke-width="0.75" stroke-linecap="round" stroke-linejoin="round"/></svg>
											</li>
							...
							<tr>
								...
								<td><strong>{{$operation->preferred_currency}} {{ app('common')->currencyNumberFormat($operation->preferred_currency, $operation->amount_requested) }}</strong></td>
							</tr>				
							<tr>
								<td>Amount Requested</td>
								<td><strong>{{$operation->preferred_currency}} {{ app('common')->currencyNumberFormat($operation->preferred_currency, $operation->amount) }}</strong></td>
							</tr>																							
						</table>
					</td>
					<td width="20%" valign="bottom">
						<div class="cheque">
							<ul>
								...
								<li><span><img src="{{ ($operation->bcp=='1') ? $active_img : $inactive_img }}"  alt="no-image"> </span>BCP</li>
								@endif
								@if($operation->inforconf != '0' && !is_null($operation->inforconf))
								<li><span><img src="{{ ($operation->inforconf=='1') ? $active_img : $inactive_img }}" alt="no-image"> </span>Inforconf</li>
								@endif
								@if($operation->infocheck != '0' && !is_null($operation->infocheck))
								<li><span><img src="{{ ($operation->infocheck=='1') ? $active_img : $inactive_img }}" alt="no-image"> </span>Infocheck</li>
								@endif
								@if($operation->criterium != '0' && !is_null($operation->criterium))
								<li><span><img src="{{ ($operation->criterium=='1') ? $active_img : $inactive_img }}" alt="no-image"> </span>Criterium</li>
								@else
								@endif
							</ul>
						</div>
				...
				<tr>
					...
					@foreach ($operation->documents as $document)
						@if($document->document_url!='')
							...
							@endphp
							@if($file_ext !='pdf')
							<td><img src="{{$document->document_url}}" alt="no-image" /></td>
							@endif
							@endif
						@endforeach
					@endif
				...
				<tr>
					...
					@foreach ($operation->supportingAttachments as $attachments)
						@if($attachments->attachment_url!='')
							...
							@endphp
							@if($file_ext !='pdf')
							<td><img src="{{$attachments->attachment_url}}" alt="no-image" /></td>
							@endif
							@endif
						@endforeach
					@endif
